import { IComment, ICommentChild } from "../types/types"

export function isValid(value: string): boolean {
  return value.length >= 1 && value.length <= 1000
}

export function compareNumberLikeChildComments(a: IComment, b: IComment): number {
  let aa = 0
  let bb = 0
...
export function getCommentsFromLocalStorage(): Array<IComment> {
  return JSON.parse(localStorage.getItem("comments") || "[]")
}

export const getLikeComments = (comments: IComment[] | []): IComment[] | [] => {
  let itog: IComment[] = []
  comments?.forEach((item) => {
    let changeChildren: ICommentChild[] = []
    if (item.children.length) {
      if (item.like === true) {
        ...
        itog.push({ ...item, children: changeChildren })
      } else {
        for (let prop in item.children) {
          if (item.children[prop]["like"] === true) {
            itog.push({
              ...item.children[prop],
              nameParent: item.nameUser,
            })
          }
        }
      }
...
export function toCard(comment: IComment): string {
  let childComments = ""
  let childComment = ""
      ...
      childComment += `
        <div class="wrapper">
          ...
          <div class="container-comment-children">
            ...
            <div>${el.text}</div>
            <div class="footer">
              <button class="like" data-id="${el.id}">${likeSVG(el.like)}${
                el.like ? "В избранном" : "В избранное"
              }</button>
  ...
  <div class="wrapper">
    ...
    <div class="container-comment">
      ...
      <div class="footer footer-parent">
        <div>
          <button class="like" data-id="${comment.id}">${likeSVG(comment.like)}${
            comment.like ? "В избранном" : "В избранное"
          }</button>
        </div>
        <div>
          ${
...
export const getProjectName = (comments: IComment[], commentId: string): IComment | ICommentChild => {
  let itog!: IComment | ICommentChild

  const findProjectName = (comments: IComment[], commentId: string): void => {
...
export function addToLocalStorage(comment: IComment | ICommentChild, idParent: string): void {
...
const firstName = new Array<string>(
  "Alice",
  "Bonnie",
  "Cassie",
  "Donna",
...
const lastName = new Array<string>(
  "Adams",
  "Bowden",
  "Conway",
  "Darden",
...
export function RandomName(): string {
  let i: number = Math.floor(Math.random() * firstName.length)
  let r: number = Math.floor(Math.random() * lastName.length)
  return firstName[i] + " " + lastName[r]
}

export const svgResponseArrow: string = `<svg class="responseArrow" xmlns="http://www.w3.org/2000/svg" width="22.702" height="21.928">
<g opacity="0.4">
...
export const likeSVG = (isLike: boolean): string => {
  return `
